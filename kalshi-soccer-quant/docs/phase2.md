...
As in Phase 1, unify everything under a **single Goalserve source**.
Because Phase 1 training data and Phase 2 inference data share a 100% identical schema,
feature-mapping errors are eliminated at the source.

| Goalserve Package | Role in Phase 2 | Core Data |
|------------------|-----------------|-----------|
| **Live Game Stats** | Lineups + formation (60 min before kickoff) | `teams.{team}.player[]`, `formation` |
| **Live Game Stats** (historical) | Player-level rolling stats | `player_stats.{team}.player[]` |
| **Fixtures/Results** (historical) | Team rolling stats, rest days, H2H | `stats.{team}`, match dates |
| **Pregame Odds** | Odds features + sanity check | 20+ bookmakers, 50+ markets |
| **Live Score** | Prepare live event ingestion | REST polling every 3s (consumed in Phase 3) |
| **Live Odds** | ob_freeze sensor + primary event-detection prep | WebSocket PUSH <1s (consumed in Phase 3) |
...
| $\boldsymbol{\delta}_H, \boldsymbol{\delta}_A$ (score-difference effect) | Step 1.4 |
...
**Goalserve live endpoints:**

```
# Lineups + player stats (60 min before kickoff)
GET /getfeed/{api_key}/soccerstats/match/{match_id}?json=1

# Same-day odds
GET /getfeed/{api_key}/soccernew/{league_id}?json=1

# Live score (for Phase 3, only connectivity validation here)
GET /getfeed/{api_key}/soccerlive/home?json=1

# Live odds (for Phase 3, only connectivity validation here)
...
**Goalserve Pregame Odds — 20+ bookmakers, 50+ markets:**

```python
def extract_odds_features(bookmakers: List[dict]) -> dict:
    """
    20+ bookmaker odds -> feature vector + sanity-check baseline values.
...
Using Goalserve Pregame Odds (20+ bookmakers, 50+ markets),
perform a more precise multidimensional validation than the original design.

### Primary Check: Match Winner (Pinnacle Baseline)
...
def primary_sanity_check(mu_H: float, mu_A: float,
    ...
    """
    Compare model probabilities vs Pinnacle + market average.
    Use Pinnacle as primary benchmark due to market efficiency.
    """
    # Model probabilities (independent Poisson)
    P_model = compute_match_odds_poisson(mu_H, mu_A)  # {H, D, A}

    # Discrepancy vs Pinnacle
    delta_pin = max(
    ...
    if delta_pin < 0.15:
        ...
        # Deviates from Pinnacle but close to market average
        # -> Pinnacle may be a temporary outlier
        if delta_mkt < 0.10:
...
Because Goalserve Pregame Odds provides 50+ markets,
...
class SanityResult:
    ...
    delta_match_winner: float   # Discrepancy vs Pinnacle
...
|   +-- ob_freeze           = False
|
+-- Quote anomaly detection (3-layer)
|   +-- P_kalshi_prev       = None  (Kalshi quote sensor)
|   +-- bet365_odds_prev    = None  (Live Odds sensor)
|   +-- bet365_score_prev   = None  (Live Odds score sensor)
|
...
|   +-- live_odds_ws        = None  (assigned after WebSocket connect)
...
|----------|---------|------|-------------|
| **Goalserve Live Odds** | **WebSocket PUSH** | **<1s** | **primary event detection + ob_freeze** |
...
**Goalserve Live Odds WebSocket connection:**

```python
async def connect_live_odds(api_key: str, match_id: str):
    """
    Connect Goalserve Live Odds WebSocket.
    Receive bet365 in-play odds + match info (score, minute, status).

    Role in Phase 3:
    1. Score-change detection (<1s) -> preliminary event
    2. Odds-jump detection -> ob_freeze
    3. Period-change detection -> halftime handling
    """
    ws = await websockets.connect(
        f"wss://goalserve.com/liveodds/{api_key}/{match_id}"
    )

    # Connectivity validation: confirm first message reception
...
### Goalserve Events -> Phase 3 Engine Mapping

**Live Odds WebSocket (primary detection, <1s):**

| Live Odds Data Change | Detection Method | Phase 3 Action |
|----------------------|----------|-------------|
| `info.score` change | compare with previous push | **PRELIMINARY goal detection + ob_freeze** |
| abrupt odds jump (>10%) | compare with previous push | **ob_freeze (goal or red card ambiguous)** |
| `info.period` change | compare with previous push | halftime enter/exit handling |
| `info.minute` > 45 or > 90 | monitor field values | stoppage-time T rolling |

**Live Score REST (authoritative confirmation, 3-8s):**
...
| period change | "1st" -> "Half" -> "2nd" | halftime cross-check |
...
|----------|----------|------|
| **Goalserve Live Odds WS disconnected** | **no heartbeat for 5s** | **disable primary detection, fallback to Live Score + Kalshi** |
...
| lineup change | recheck before kickoff | rerun Steps 2.1-2.3 |
| match postponed/cancelled | Goalserve status update | full shutdown, flatten positions |

> **Graceful degradation:** Live Odds WebSocket has been promoted from supplementary to core,
...
| $\boldsymbol{\delta}_H[5], \boldsymbol{\delta}_A[5]$ | score-difference effect | Phase 1 |
...
| **Goalserve Live Odds WS** | **primary event detection + ob_freeze** | Step 2.5 |
| Goalserve Live Score | REST polling ready | Step 2.5 |
| Kalshi WS | quote + trading ready | Step 2.5 |
| ob_freeze | False (3-layer sensors initialized) | Step 2.5 |
...
|  2.1.5: Pregame Odds -> 20+ bookmaker odds features          |
...
|  • Primary: Match Winner vs Pinnacle + market average         |
...
|  • 3-layer ob_freeze sensors initialized                      |
...
+--------------------------------------------------------------+
        ...
        (3-layer detection: Live Odds WS + Kalshi WS + Live Score REST)
```
